...
export const keys = pgTable("keys", {
  ...
  hwidResetCount: integer("hwid_reset_count").default(0).notNull(),
  source: text("source"),
  ...
  loaderScript: text("loader_script"),
  notes: text("notes"),
...
export const botGenerateKeySchema = z.object({
  durationDays: z.number().min(1).max(3650),
  notes: z.string().max(500).optional(),
});

export const botRevokeKeySchema = z.object({
  key: z.string().min(1, "Key is required"),
});
...
}).refine((v) => v.isLifetime === true || v.durationMonths != null || v.durationDays != null, {
  message: "Duration is required",
  path: ["durationMonths"],
});

export const compensateSchema = z.object({
  hours: z.number().int().positive("Minimal 1 jam kompensasi"),
});
