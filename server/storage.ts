...
export interface IStorage {
  ...
  getOtp(userId: string, purpose: string, code: string): Promise<Otp | undefined>;
  deleteOtps(userId: string, purpose: string): Promise<void>;
  ...
  getKeysByDiscordId(discordId: string): Promise<Key[]>;
  updateKeyDiscordId(keyId: number, discordId: string | null): Promise<boolean>;
  ...
  addCompensation(hours: number): Promise<number>;
  ...
  getTestimonialsPaginated(limit: number, offset: number, filters?: { status?: string }): Promise<(Testimonial & { user?: User })[]>;
  getTestimonialsTotal(filters?: { status?: string }): Promise<number>;
...
export class DatabaseStorage implements IStorage {
  ...
  async verifyUserEmail(id: string): Promise<void> {
    await db.update(users).set({ isEmailVerified: 1 }).where(eq(users.id, id));
  }
  ...
  async getOtp(userId: string, purpose: string, code: string): Promise<Otp | undefined> {
    const [found] = await db.select().from(otps).where(and(eq(otps.userId, userId), eq(otps.purpose, purpose), eq(otps.code, code)));
    return found || undefined;
  }

  async deleteOtps(userId: string, purpose: string): Promise<void> {
  ...
  async getKeysByDiscordId(discordId: string): Promise<Key[]> {
    return db
      .select()
      .from(keys)
      .where(eq(keys.discordId, discordId))
      .orderBy(desc(keys.createdAt));
  }

  async updateKeyDiscordId(keyId: number, discordId: string | null): Promise<boolean> {
    const [updated] = await db
      .update(keys)
      .set({ discordId })
  ...
  async addCompensation(hours: number): Promise<number> {
    const result = await db.update(keys)
      .set({
        expiresAt: sql`${keys.expiresAt} + interval '1 hour' * ${hours}`
      })
      .where(
        and(
          eq(keys.status, "active"),
          isNotNull(keys.expiresAt)
        )
      )
  ...
  async getAllTestimonials(): Promise<(Testimonial & { user?: User })[]> {
    const records = await db.query.testimonials.findMany({
  ...
  async getTestimonialsPaginated(limit: number, offset: number, filters?: { status?: string }): Promise<(Testimonial & { user?: User })[]> {
    let condition = undefined;
    if (filters?.status && filters.status !== "all") {
      condition = eq(testimonials.status, filters.status);
    }
  ...
  async getTestimonialsTotal(filters?: { status?: string }): Promise<number> {
    let condition = undefined;
    if (filters?.status && filters.status !== "all") {
      condition = eq(testimonials.status, filters.status);
    }
    
    const result = await db.select({ count: sql<number>`count(*)::int` }).from(testimonials).where(condition);
    return result[0]?.count ?? 0;
  }
