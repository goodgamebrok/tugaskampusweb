...
const generateSchema = z.object({
  durationKind: z.enum(["months", "days", "lifetime"]),
  ...
  // lifetime doesn't need duration validation
});
...
export default function Generate() {
  ...
  const onSubmit = (data: GenerateFormData) => {
    let payload: { isLifetime?: boolean; durationMonths?: number; durationDays?: number; price: string; quantity: number; packageId?: number; notes?: string };
    if (data.durationKind === "lifetime") {
      payload = {
        isLifetime: true,
        price: data.price,
    ...
    } else if (data.durationKind === "days") {
      payload = {
          ...
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <RadioGroup
                          value={field.value}
                          onValueChange={(val) => field.onChange(val as "months" | "days" | "lifetime")}
                        ...
                        >
                          ...
                          <div>
                            <RadioGroupItem value="lifetime" id="duration-kind-lifetime" className="peer sr-only" />
                            <Label
                              htmlFor="duration-kind-lifetime"
                              className="flex cursor-pointer flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-3 hover-elevate peer-data-[state=checked]:border-amber-500 peer-data-[state=checked]:bg-amber-500/10"
                            >
                              <span className="font-medium">♾️ Lifetime</span>
                            </Label>
                          </div>
                        </RadioGroup>
                      </FormControl>
                ...
                {form.watch("durationKind") === "lifetime" ? (
                  <div className="rounded-lg border-2 border-amber-500/30 bg-amber-500/5 p-4 text-center">
                    <p className="text-sm font-medium text-amber-400">♾️ Key ini tidak akan pernah expire (Permanent)</p>
                  </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>Harga per key dalam Rupiah</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
